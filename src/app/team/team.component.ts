...
export class TeamComponent {
  ...
  public onSelection(e: Event) {
    ...
    this.selectedPokemonsIds = this.selectedPokemonsIds.filter(
      (id) => id !== Number(selectedPokemonId)
    );
  ...
  public submitTeam() {
    if (this.selectedPokemonsIds.length !== 6 || !this.teamName) {
      console.log('missing input');
      return;
    }
    let body = {
      teamName: this.teamName,
      pokemonIds: this.selectedPokemonsIds,
    };
